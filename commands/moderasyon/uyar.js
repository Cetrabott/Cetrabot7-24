...
module.exports = class ModerationWarnCommand extends Command {
	...
	async run(msg, args) {
		...
		if (db ==! "evet") return msg.channel.send(`${this.client.emojis.get('469536508479799296')}` + ' Lütfen `mod-log-ayarla` komutu ile mod-log kanalı belirleyiniz.');
		let modlog = vt;
		if (!modlog) return msg.channel.send(`${this.client.emojis.get('469536508479799296')}` + ' Mod-log olarak belirlediğiniz kanal silinmiş, lütfen yeni  bir mod-log kanalı açıp `mod-log-ayarla` komutu ile mod-log olarak ayarlayınız.');
		if (user.id === msg.author.id) return msg.say(`${this.client.emojis.get('469536508479799296')}` + ' Kendini uyaramazsın.')
		if (member.highestRole.calculatedPosition > msg.member.highestRole.calculatedPosition - 1) 
		{ return msg.say(`${this.client.emojis.get('469536508479799296')}` + ' Bu kişinin senin rollerinden/rolünden daha yüksek rolleri/rolü var.');
		}
		if (!msg.guild.member(user).kickable) return msg.channel.send(`${this.client.emojis.get('469536508479799296')}` + ' Bu kişiyi uyaramıyorum çünkü `benden daha yüksek bir role sahip` ya da `bana gerekli yetkileri vermedin`.');
		...
		return msg.channel.send(`${this.client.emojis.get('469536508479799296')}` + ' İşlem başarılı!');
	}
};
