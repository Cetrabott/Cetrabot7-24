...
module.exports = class BlacklistUserCommand extends Command {
	constructor(client) {
		super(client, {
			...
			aliases: ['hoşgeldinayarla', 'hoşgeldin', 'hoş-geldin', 'hos-geldin-ayarla'],
			group: 'ayarlar',
	...
	async run(msg, args) {
			...
			if (vt === args.kanal.id) {
				...
				msg.channel.send(`Hoş geldin kanalı zaten **${args.kanal.name}** olarak ayarlı.`);
			} else {
				this.client.provider.set(msg.guild.id, 'hosGeldin', args.kanal.id);
				this.client.provider.set(msg.guild.id, 'hosGeldinK', true);
				return msg.channel.send(`Hoş geldin olarak ayarlanan kanal: **${args.kanal.name}**`);
			}
	}
};