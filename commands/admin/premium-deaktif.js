...
module.exports = class PreEnableCommand extends Command {
	...
	async run(msg, args) {
	    ...
	    if (this.client.guilds.has(sunucu)) {
    		...
    		if (preEnabled !== true) return msg.channel.send(' bu sunucu zaten premium bota sahip değil.');
    		...
    		return this.client.guilds.get(sunucu).leave().then(() => mesaj.edit(` Başarıyla ${sunucuAdi} sunucusunda premium deaktifleştirildi.`));
    	} else {
    		const preEnabled = this.client.provider.get(sunucu, 'preEnabled', []);
    		if (preEnabled !== true) return msg.channel.send(' bu sunucu zaten premium bota sahip değil.');
